...
from octagon.utils import dual_rect_anchor, debug_outlines, mask_overlay, cord_to_block, var
...
class Entity(pygame.sprite.Sprite):
    ...
    def _am_settarget(self):
        target = [self.am_path[self.am_todo][0]-self.floorjson["size"], self.am_path[self.am_todo][1]-self.floorjson["size"]]
    ...
    def _am_newpath(self):
        ...
        if self.am_target != self.position:
            ...
            self._am_settarget()
        # print(self.am_grid.grid_str(path=self.am_path, start=start, end=end))
