...
def _show_inventory(window, background):
    ...
    for i in range(5):
        for j in range(6):
            ...
            images.append(image.Image(tags=["overlay", "tab", i*6+j], image=img.misc["inventory"]["attack"][0], relpos=(0.605 + j / 14.25, 0.25 + i / 8), h_event=True, h_image=
            img.misc["inventory"]["attack"][1]))
    for i in range(len(inventory["hotbar"])):
        images.append(image.Image(tags=["overlay", "hotbar", i], image=img.misc["inventory"]["unset"][0], relpos=(0.605 + i / 14.25, 0.925), h_event=True, h_image=
        img.misc["inventory"]["unset"][1]))

    # add items
    for i in range(len(inventory["inventory"])):
        for j in range(len(inventory["inventory"][i][1])):
            if inventory["inventory"][i][1][j][1] != "unset":
                images.append(image.Image(tags=["tab", inventory["inventory"][i][0], j, inventory["inventory"][i][1][j][1], inventory["inventory"][i][1][j][2], inventory["inventory"][i][1][j][3]], image=
                img.item[inventory["inventory"][i][1][j][1]], relpos=(slots[j])))
    ...
    for i in range(len(inventory["hotbar"])):
        if inventory["hotbar"][i][1] != "unset":
            images.append(image.Image(tags=["hotbar", inventory["hotbar"][i][0], i, inventory["hotbar"][i][1], inventory["hotbar"][i][2], inventory["hotbar"][i][3]], image=
            img.item[inventory["hotbar"][i][1]], relpos=(0.605 + i / 14.25, 0.925)))
            for j in images:
    ...
    inv_gui = gui.GUI(background=background, overlay=200, labels=labels, images=images, buttons=[
        button.Button(tags=["attack", ""], anchor="center", relsize=(0.17, 0.1), text="WEAPON", relpos=(0.1, 0.1)),
